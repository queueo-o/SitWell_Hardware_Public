...
SENSOR_TYPE = {
    ...
    "fsr2": "circle",          # A1 — circle FSR
    "fsr3": "square",
    "fsr4": "square",         # A0 — circle FSR
    "fsr5": "velostat",       # A1 — square FSR
    "fsr6": "velostat",       # A2 — velostat / conductive sheet
}
...
DEVIATION = {
    "circle":   0.6,        # ±25%  →  >1.25x or <0.75x baseline
    "square":   0.7,         # ±25%  →  >1.25x or <0.75x baseline
    "velostat": 0.2,         # ±10%  →  >1.10x or <0.90x baseline
}
...
VELOSTAT_THRESHOLDS = [
    (100,  "No pressure"),         # sensor at rest, no contact
    (200,  "Below normal"),        # between rest and good-posture band
    (300,  "Normal posture"),      # target zone
...
ble.name = "Posture Check BT Daniel"
uart = UARTService()
...
def check_posture(uart_svc, r1, r2, r3, r4, r5, r6):
    """
...
while True:
    ...
    while ble.connected:
        ...
        if calibration["is_calibrated"] and (now - last_monitor_time >= MONITOR_INTERVAL):
            ...
            alerts = check_posture(uart, r1, r2, r3, r4, r5, r6)
